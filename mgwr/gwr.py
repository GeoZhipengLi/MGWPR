...
# from statsmodels.sandbox.archive.linalg_covmat import sigma2

from .diagnostics import get_AIC, get_AICc, get_BIC, corr

from .kernels import Kernel, local_cdist

from .summary import summaryModel, summaryGLM, summaryGWR, summaryACK, summaryAbout, summaryMGWR
...
class GWR(GLM):
    ...
    def __init__(self, coords, y, X, bw, time, family=Gaussian(), offset=None,
        ...
        self.time = time
        self.temp_inv_xtx_xt = [None] * int(self.y.shape[0] / self.time)
        self.temp_wi = [None] * int(self.y.shape[0] / self.time)
    ...
    def _local_fit(self, i):
        ...
        #print(len(self.temp_wi))
        position_id = i // self.time
        #print(position_id)
        if self.temp_wi[position_id] is None :
            wi = self._build_wi(i, self.bw).reshape(-1, 1)  # local spatial weights
            #wi = [x/self.time for x in wi]
            self.temp_wi[position_id] = wi
        else:
            wi = self.temp_wi[position_id]

        if isinstance(self.family, Gaussian):
            if self.temp_inv_xtx_xt[position_id] is None:
                xT = (self.X * wi).T
                xtx = np.dot(xT, self.X)
                inv_xtx_xt = linalg.solve(xtx, xT)
                self.temp_inv_xtx_xt[position_id] = inv_xtx_xt
                ...
                inv_xtx_xt = self.temp_inv_xtx_xt[position_id]
                betas = np.dot(inv_xtx_xt, self.y)
                # betas, inv_xtx_xt = _compute_betas_gwr(self.y, self.X, wi)
                predy = np.dot(self.X[i], betas)[0]
                resid = self.y[i] - predy
            ...
            betas = np.dot(xtx_inv_xt, self.y)
        ...
        # if not lite, return the full set of results
        Si = np.dot(self.X[i], inv_xtx_xt).reshape(-1)
        tr_STS_i = np.sum(Si * Si * w * w)
        cct_cct = np.dot(inv_xtx_xt, inv_xtx_xt.T)
        CCT = np.diag(cct_cct).reshape(-1)
        if not self.hat_matrix:
            Si = None
        return influ, resid, predy, betas.reshape(-1), cct_cct, w, Si, tr_STS_i, CCT
    ...
    def fit(self, ini_params=None, tol=1.0e-5, max_iter=20, solve='iwls',
        ...
        # if pool:
        #     warnings.warn(
        #         "The pool parameter is no longer used and will have no effect; parallelization is default and implemented using joblib instead.", RuntimeWarning, stacklevel=2)
        ...
        if solve.lower() == 'iwls':
            ...
            cct_cct = np.array(rslt_list[-5])
            cct_cct = cct_cct[:, 1:, 1:]
            return GWRResults(self, params, predy, S, CCT, influ, tr_STS, w, self.name_x, cct_cct)
    ...
    def predict(self, points, P, exog_scale=None, exog_resid=None,
        ...
        if (exog_scale is None) & (exog_resid is None):
            ...
            #                              'None or specified')
            raise Exception('exog_scale and exog_resid must both either be'
                             'None or specified')
        self.points = points
        if self.constant:
...
class GWRResults(GLMResults):
    ...
    def __init__(self, model, params, predy, S, CCT, influ, tr_STS=None,
                 w=None, name_x=None, cct_cct=None):
        ...
        self._cache = {}
        self.name_x = name_x
        self.cct_cct = cct_cct
        self.varai = None  # Use None instead of an empty list


    def var_ai(self, X_mean, time):
        self.cct_cct = self.cct_cct * self.sigma2  # Scale cct_cct
        # Initialize varai as a NumPy array of zeros (for better efficiency)
        num_matrices = self.cct_cct.shape[0]  # Number of matrices
        self.varai = np.zeros(num_matrices)  # Store results in an array

        for i, varb in enumerate(self.cct_cct):
            xi_mean = X_mean[i, :]
            self.varai[i] = xi_mean @ varb @ xi_mean.T  # Store result directly

        self.varai += self.sigma2 / time  # Apply the final operation in one step
    ...
    def get_bws_intervals(self, selector, pval=0.95):
        ...
        aiccs = pd.DataFrame(list(zip(*selector.sel_hist))
                             [1], columns=["aicc"])
        ...
        # Get bw boundaries
        interval = (aiccs.iloc[:index, :].bw.min(),
                    aiccs.iloc[:index, :].bw.max())
        return interval
...
class MGWR(GWR):
    ...
    def __init__(self, coords, y, X, selector, time, sigma2_v1=True,
        ...
        GWR.__init__(self, coords, y, X, self.bw_init, self.time, family=self.family,
    ...
    def fit(self, n_chunks=1, pool=None):
        ...
        return MGWRResults(self, params, predy, CCT, ENP_j, w, R, name_x=self.name_x)
    ...
    def exact_fit(self):
        ...
        for j1 in range(self.k):
            Aj = GWR(self.coords, self.y, self.X[:, j1].reshape(
                -1, 1), bw=self.bws[j1], hat_matrix=True, constant=False, n_jobs=self.n_jobs).fit().S
            Pj = []
        ...
        for j in range(self.k):
            CCT[:, j] = (
                (R[:, :, j] / self.X[:, j].reshape(-1, 1))**2).sum(axis=1)
...
class MGWRResults(GWRResults):
    ...
    def __init__(self, model, params, predy, CCT, ENP_j, w, R, name_x=None):
        ...
        self.R = R
        self.name_x = name_x
        GWRResults.__init__(self, model, params, predy, None, CCT, None, w, name_x=name_x)
        if model.hat_matrix:
            self.S = np.sum(self.R, axis=2)
        self.predy = predy

    def ai(self, X_mean, Y_mean, X_scale):
        params = self.params * X_scale
        x_params = X_mean * params
        ai = Y_mean - x_params.sum(axis=1).reshape(-1, 1)
        return ai

    def std_ai(self, X_mean, time, bws, coords, X, X_scale, Y_std, fixed=False, spherical=False, kernel='bisquare', points = None):

        varai = np.zeros(X.shape[0])  # Store results in an array
        bw_list = bws
        X_scale2 = np.outer(X_scale, X_scale)
        coords = np.array(coords)
        # Loop over covariates (rows in X)
        for i in range(X.shape[0]):
            cct = []  # List to store computations
            for j in range(X.shape[1]):
                # Get the weight vector for the j-th covariate
                bw = bw_list[j] * time
                wi = Kernel(i, coords, bw, fixed=fixed,function=kernel, points=points, spherical=spherical).kernel
                #wi = [x / time for x in wi]
                xj = X[:,j]
                xt = xj * wi
                xtx = np.sum(xt*xj)

                inv_xtx_xt = xt / xtx
                cct.append(inv_xtx_xt)

            # Calculate variance for the current observation
            cct = np.array(cct)

            varbi = np.dot(cct, cct.T) * self.sigma2 # Variance matrix
            varbi =  varbi * X_scale2
            xi_mean = X_mean[i, :]
            varai[i] =  np.dot(np.dot(xi_mean, varbi), xi_mean.T)  # xi_mean @ varbi @ xi_mean.T

        orign_sigma2 = (Y_std * Y_std * self.sigma2) * time / (time - 1)
        varai += orign_sigma2 / time
        std_ai = np.sqrt(varai)
        return std_ai
    ...
    def get_bws_intervals(self, selector, pval=0.95):
        ...
        for j in range(self.k):
            # Get AICcs and associated bw from the last iteration of back-fitting and make a DataFrame
            aiccs = pd.DataFrame(
                list(zip(*selector.sel_hist[-self.k+j]))[1], columns=["aicc"])
    ...
    def spatial_variability_fast(self, selector, n_iters=1000, seed=None, pool=os.cpu_count()) -> list:
        """
        Method to compute a Monte Carlo test of spatial variability for each
        estimated coefficient surface.
        ...
        seed            : int
                          ...
                          which automatically sets the seed to 5536
        pool            : int
                          The number of jobs (default system cpus) to run in parallel. -1 means using all processors.
        ...
        with Pool(pool) as p:
            SDs = p.map(self.mc_iter, range(n_iters))
