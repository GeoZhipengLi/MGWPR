...
class Sel_BW(object):
    ...
    def __init__(self, coords, y, X_loc, time, X_glob=None,  family=Gaussian(),
    ...
    def search(self, search_method='golden_section', criterion='AICc',
        ...
        #zhipeng
        self.bw_max = (self.X_loc.shape[0] / self.time)
        ...
        # Can not change the return setting!! Because it will be used in the following iterations.
        return self.bw[0]

    def _bw(self):
        #print(1)
        def gwr_func(bw): return getDiag[self.criterion](GWR(
            self.coords, self.y, self.X_loc, bw,
            family=self.family, kernel=self.kernel,
            fixed=self.fixed, constant=self.constant,
            offset=self.offset, spherical=self.spherical,
            n_jobs=self.n_jobs, time = self.time).fit(lite=True, pool=self.pool))

        self._optimized_function = gwr_func

        if self.search_method == 'golden_section':
            self.bw_min, self.bw_max = self._init_section(self.X_glob,
                                                          self.X_loc,
                                                          self.coords,
                                                          self.constant, self.time)
            delta = 0.38197  # 1 - (np.sqrt(5.0)-1.0)/2.0

            # zhipeng
            #self.bw_min = int(self.bw_min / self.time)
            #self.bw_max = int(self.bw_max / self.time)
            ...
            self.bw_min, self.bw_max = self._init_section(
                self.X_glob, self.X_loc, self.coords, self.constant, self.time)
    ...
    def _mbw(self):
        ...
        def gwr_func(y, X, bw):
            return GWR(coords, y, X, bw, family=family, kernel=kernel,
                       ...
                       spherical=self.spherical, hat_matrix=False, time=self.time).fit(
                           lite=True, pool=self.pool)

        def bw_func(y, X):
            selector = Sel_BW(coords, y, X, time= self.time, X_glob=[], family=family,
        ...
        self.bw = multi_bw(self.init_multi, y, X, XT, n, k, family, self.tol_multi,
                           self.max_iter_multi, self.rss_score, gwr_func,
                           bw_func, sel_func, multi_bw_min, multi_bw_max,
                           bws_same_times, verbose=self.verbose)

    def _init_section(self, X_glob, X_loc, coords, constant, time):
