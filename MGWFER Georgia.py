...
Y = data['bachelor_within'].values.reshape(-1, 1)
Y_std = Y.std()
X = data[["lnpop_within", "black_within","fb_within","rural_within", "poverty_within", "income_within"]].values
...
Y_mean = data['bachelor_mean'].values.reshape(-1, 1)
X_mean = data[["lnpop_mean", "black_mean","fb_mean","rural_mean", "poverty_mean", "income_mean"]].values
...
t_value = ai / std_ai
p_values = 2 * (1 - t.cdf(np.abs(t_value), df= 630))
adjusted_level = 0.05
...
np.savetxt('Output/parametersG.csv', model.params, delimiter=',')
np.savetxt('Output/sigG.csv', sig, delimiter=',')
np.savetxt('Output/aiG.csv', ais, delimiter=',')
